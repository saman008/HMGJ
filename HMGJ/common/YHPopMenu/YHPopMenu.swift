...
let kGreenColor = BColor
let kGrayColor_Swift = BColor
...
class YHPopMenu : UIView,UITableViewDelegate,UITableViewDataSource{
    ...
    open func show(){
        var viewBGH = ScreenH - 64.0 - 44.0
        if canTouchTabbar == false {
            viewBGH = ScreenH - 64.0
        }
        
        self.frame = CGRect(x: 0, y: 64, width: ScreenW, height: viewBGH)
