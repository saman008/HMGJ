...
@property (nonatomic ,strong) NSString * stateStr1;       //选中的运营状态位置Int
...
- (void)setDataSource:(NSArray *)dataSource
{
    ...
    self.stateStr1 = [NSString stringWithFormat:@"%@",@(1)];
    //刷新布局
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    self.stateStr1 = [NSString stringWithFormat:@"%@", @(row + 1)];
}
...
- (void)finishBtnClicked:(UIButton *)button
{
    self.valueDidSelect(self.stateStr);
     self.valueDidSelect1(self.stateStr1);
